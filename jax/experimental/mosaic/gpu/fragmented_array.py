...
class Tiling:
  ...
  def tile_shape(self, shape: tuple[int, ...]) -> tuple[int, ...]:
    """Computes the shape of an array after tiiling."""
    def fail():
  ...
  def tile_strides(self, strides: tuple[int, ...]) -> tuple[int, ...]:
    """Computes the strides of an array after tiiling."""
    for tile in self.tiles:
...
class TiledLayout:
  ...
  split across warps (within the warpgroup), lanes, and vectorized according to
  ...
  def shape_from_registers(self, shape: tuple[int, ...]) -> tuple[int, ...]:
  ...
  def lane_indices(self) -> tuple[ir.Value, ...]:
    ...
    lane_idx = arith.remui(utils.thread_idx(), c(32, i32))
...
# WGMMA layout for 16-bit data, expressed as a tiled layout.
...
class FragmentedArray:
  ...
  def __init__(
    ...
    match self.layout:
      ...
      case TiledLayout():
        try:
          self.layout.shape_from_registers(_registers.shape)
        except ValueError:
          raise ValueError(
  ...
  def shape(self):
    match self.layout:
      ...
      case WGSplatFragLayout(shape=shape):
        return shape
      case TiledLayout():
        return self.layout.shape_from_registers(self.registers.shape)
      case _:
        raise NotImplementedError
  ...
  def to_layout(self, new_layout: FragmentedLayout):
    ...
    if self.layout == new_layout:
      return self
    shape = self.shape
    if {self.layout, new_layout} == {WGMMA_LAYOUT, _tiled_wgmma_layout(shape)}:
  ...
  def transfer_tiled2(
    ...
    dot = lambda x, y: sum(x_ * y_ for x_, y_ in zip(x, y))
    for tile_idx in np.ndindex(*tiled_shape):
      const_offset = dot(tile_idx, tiled_strides)
