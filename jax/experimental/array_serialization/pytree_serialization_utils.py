...
class SerializationRegistry:
  """
  A registry for serializing and deserializing objects:
    - leaf objects in a pytree
    - aux data of a pytreedef node
  """
  def __init__(self):
    self._in_place = {}
  ...
  def with_fallback(self, dumps: Callable[[Any], str | bytes],
    ...
    """
    Context manager that registers the given serialization and deserialization
    functions as fallback for unregistered types.
    
    This does not help with undefined classes, we will not attempt to import
    them dynamically.
...
def _node_deserialize(node_data_store: dict[str, Any],
  ...
  try:
    ...
    if node_type is MISSING:
      raise ValueError(
          f"Node type \"{type_repr}\" is not registered. Register it via"
    ...
    if node_id is None:
      raise ValueError(f"Node type {type_repr} is not registered as in-tree,"
                        ...
                        f" use argument `best_effort=True` to reconstruct this"
                        f" node as a list of children.")
