...
bool canReinterpretToUntiledMemref(TypedValue<MemRefType> tiled_memref,
  ...
  if (!tiled_layout) {
    if (auto erase_op = tiled_memref.getDefiningOp<tpu::EraseLayoutOp>()) {
      ...
      tiled_layout =
          dyn_cast<tpu::TiledLayoutAttr>(tiled_memref_ty.getLayout());
      if (auto slice_op = tiled_memref.getDefiningOp<tpu::MemRefSliceOp>()) {
  ...
  if (tiled_memref_ty.getNumDynamicDims() != dynamic_sizes.size()) {
    ...
    // eraseLayout` to get the dynamic sizes, but other patterns won't work
    // here: eg., slice -> eraseLayout -> reshape -> eraseLayout`. We should fix
    // this! For now, if we can not get the expected dynamic sizes, we consider
    // the memref cannot be reinterpreted to untiled.
    return false;
  }
